...
def extract_data():
    ...
    objects_to_delete = s3.list_objects_v2(Bucket=BUCKET_NAME, Prefix='original_data/')
    ...
    try:
        old_obj = s3.get_object(Bucket=BUCKET_NAME, Key=outdated_key)
    ...
    s3.put_object(Bucket=BUCKET_NAME, Key='original_data/austin_animal_outcomes.csv', Body=file_content)
    ...
    s3.put_object(Bucket=BUCKET_NAME, Key=outdated_key, Body=old_file_content)
...
def transform_data():
    ...
    s3_object_name = 'original_data/austin_animal_outcomes.csv'
    ...
    s3.put_object(Bucket=bucket_name, Key='cleaned/data.csv', Body=out_buffer.getvalue())

    out_buffer = io.StringIO()
    animal.drop_duplicates(inplace = True)
    animal.to_csv(out_buffer, index=False)
    s3.put_object(Bucket=bucket_name, Key='cleaned/animal.csv', Body=out_buffer.getvalue())
  
    out_buffer = io.StringIO()
    outcomes.drop_duplicates(inplace = True)
    outcomes.to_csv(out_buffer, index=False)
    s3.put_object(Bucket=bucket_name, Key='cleaned/outcomes.csv', Body=out_buffer.getvalue())

    out_buffer = io.StringIO()
    outcome_events.drop_duplicates(inplace = True)
    outcome_events.to_csv(out_buffer, index=False)
    s3.put_object(Bucket=bucket_name, Key='cleaned/outcomes_events.csv', Body=out_buffer.getvalue())
...
def load_data():
    ...
    files = ['cleaned/animal.csv', 'cleaned/outcomes.csv', 'cleaned/outcomes_events.csv', 'cleaned/data.csv']
    dataframes = {}
    for file in files:
    ...
    dataframes['cleaned/animal.csv'].to_sql('animal', engine, if_exists='append', index=False)
    dataframes['cleaned/outcomes.csv'].to_sql('outcome_type', engine, if_exists='append', index=False)
    dataframes['cleaned/outcomes_events.csv'].to_sql('outcome_event', engine, if_exists='append', index=False)
    dataframes['cleaned/data.csv'].to_sql('fact_table', engine, if_exists='append', index=False)
